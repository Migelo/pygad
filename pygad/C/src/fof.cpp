...
extern "C" void find_fof_groups(size_t N, double *pos, double *vel,
                                double *mass, double l, double dvmax,
                                size_t min_parts, int sort, size_t *FoF,
                                double periodic, void *octree) {
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    while (friends.size()) {
      ...
      std::vector<size_t> new_friends = tree->ngbs_within_if(
          pos + 3 * j, l, pos, periodic, [&FoF, &vel, &j, &dvmax2](size_t idx) {
            return FoF[idx] == NOT_PROCESSED &&
                   dist2<3>(vel + 3 * j, vel + 3 * idx) < dvmax2;
          });
  ...
  if (sort) {
    ...
    std::sort(group.begin(), group.end(), [&FoF_mass](size_t i, size_t j) {
      return FoF_mass[i] > FoF_mass[j];
    });

    /*
    printf("The %zu biggest FoF halos:\n", std::min<size_t>(FoF_mass.size(),
    10)); for (size_t i=0; i<std::min<size_t>(FoF_mass.size(), 10); i++) {
