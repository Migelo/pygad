...
void _absorption_spectrum_multiple_los(
    size_t N,       // number of particles/cells
    double *pos,    // [x,y] positions of the particles
    double *vel,    // z-velocity of the particles
    double *vpec_z, // DS: LOS peculiar velocity array
    double *hsml,   // softening length of the particles/cells
    double *n,      // number of ions per particle
    double *temp,
    double *rho,         // DS: density array
    double *metal_frac,  // SA: metal mass fraction
    double *los_pos_arr, // array of line-of-sight [x,y] positions
    double *vel_extent,  // velocity extent of the line profile
    size_t Nbins, size_t Nlos, double b_0, double *v_turb, double Xsec,
    double Gamma, double *taus, double *los_dens,
    ...
    double *v_lims, double *column, const char *kernel_, double periodic) {
  ...
  std::memset(los_dens_phys, 0,
              Nlos * Nbins * sizeof(double)); // DS: density array
  std::memset(los_metal_frac, 0,
              Nlos * Nbins * sizeof(double)); // SA: LOS metal mass fraction
  std::memset(los_temp, 0, Nlos * Nbins * sizeof(double));
  std::memset(los_vpec, 0,
              Nlos * Nbins * sizeof(double)); // DS: LOS peculiar velocity field
...
#pragma omp parallel for default(shared) schedule(dynamic, 20)
  for (size_t los_idx = 0; los_idx < Nlos; los_idx++) {
    ...
    for (size_t j = 0; j < N; j++) {
      ...
      double FWHM_V = 0.5346 * FWHM_L +
                      std::sqrt(0.5346 * FWHM_L * FWHM_L + FWHM_b * FWHM_b);
      ...
      if (Gamma > 0.0) {
        ...
        vi_min = std::max<double>(
            0.0, std::floor((vj - v_width - vel_extent[0]) / dv));
        vi_max = std::min<double>(
            Nbins - 1, std::ceil((vj + v_width - vel_extent[0]) / dv));
      }
...
#pragma omp atomic
        los_dens_phys[los_idx * Nbins + vi_min] +=
            Rhj * Nj; // DS: density skewer
#pragma omp atomic
        los_temp[los_idx * Nbins + vi_min] += Tj * Nj;
#pragma omp atomic
        los_vpec[los_idx * Nbins + vi_min] +=
            vzj * Nj; // DS: LOS peculiar velocity field
#pragma omp atomic
        los_metal_frac[los_idx * Nbins + vi_min] +=
            Zj * Nj; // SA: LOS metal mass fraction
        ...
        for (size_t i = vi_min; i <= vi_max; i++) {
          ...
          if (FWHM_V < 10. * dv) {
            ...
            if (Gamma > 0.0) {
              ...
              Dtb *= 2;
              Dtb += Voigt(v0, sigma, Gamma) + Voigt(v1, sigma, Gamma);
              Dtb *= h / 3.;
            } else {
            ...
            if (Gamma > 0.0) {
              ...
              Dtb =
                  std::exp(-std::pow(Dv / b, 2.0)) / (b * std::sqrt(M_PI)) * dv;
            }
          }
          double DtbNj = Dtb * Nj;
...
#pragma omp atomic
          los_dens_phys[los_idx * Nbins + i] +=
              Rhj * DtbNj; // DS: density skewer
#pragma omp atomic
          los_temp[los_idx * Nbins + i] += Tj * DtbNj;
#pragma omp atomic
          los_vpec[los_idx * Nbins + i] +=
              vzj * DtbNj; // DS: LOS peculiar velocity field
#pragma omp atomic
          los_metal_frac[los_idx * Nbins + i] +=
              Zj * DtbNj; // SA: LOS metal mass fraction
    ...
    for (size_t i = 0; i < Nbins; i++) {
      ...
      if (los_dens[los_idx * Nbins + i] != 0.0) {
        ...
        los_dens_phys[los_idx * Nbins + i] /=
            los_dens[los_idx * Nbins + i]; // DS: density skewer
        los_vpec[los_idx * Nbins + i] /=
            los_dens[los_idx * Nbins + i]; // DS: LOS peculiar velocity field
        los_metal_frac[los_idx * Nbins + i] /=
            los_dens[los_idx * Nbins + i]; // SA: LOS metal mass fraction
      }
    }
  }
}
...
template <bool particles>
void _absorption_spectrum(size_t N, double *pos, double *vel,
                          double *vpec_z, // DS: LOS peculiar velocity array
                          double *hsml, double *n, double *temp,
                          double *rho,        // DS: density array
                          double *metal_frac, // SA: metal mass fraction
                          double *los_pos, double *vel_extent, size_t Nbins,
                          double b_0, double *v_turb, double Xsec, double Gamma,
                          double *taus, double *los_dens,
                          double *los_dens_phys,  // DS: density array
                          double *los_metal_frac, // SA: LOS metal mass fraction
                          double *los_temp,
                          double *los_vpec, // DS LOS peculiar velocity field
                          double *v_lims, double *column, const char *kernel_,
                          double periodic) {
  double dv = (vel_extent[1] - vel_extent[0]) / Nbins;
  Kernel<3> &kernel = kernels.at(kernel_);
  if (particles) {
  ...
  std::memset(los_metal_frac, 0,
              Nbins * sizeof(double)); // SA: LOS metal mass fraction
  std::memset(los_temp, 0, Nbins * sizeof(double));
  std::memset(los_vpec, 0,
              Nbins * sizeof(double)); // DS: LOS peculiar velocity field
  ...
  // std::mt19937 gen(rd());
  // std::uniform_real_distribution<double> dis(0.5, 110.0);

#pragma omp parallel for default(shared) schedule(dynamic, 10)
  // for(int los_idx = 0; los_idx < N_los; los_idx++)
  //   {
  //     double random_number1 = dis(gen);
  //     double random_number2 = dis(gen);
  //     los_pos[0]            = random_number1;
  //     los_pos[1]            = random_number2;
  for (size_t j = 0; j < N; j++) {
    ...
    double FWHM_b = 2 * std::sqrt(2 * std::log(2)) * sigma;
    double FWHM_V =
        0.5346 * FWHM_L + std::sqrt(0.5346 * FWHM_L * FWHM_L + FWHM_b * FWHM_b);
    ...
    if (Gamma > 0.0) {
      ...
      vi_min = std::max<double>(
          0.0, std::floor((vj - v_width - vel_extent[0]) / dv));
      vi_max = std::min<double>(Nbins - 1,
                                std::ceil((vj + v_width - vel_extent[0]) / dv));
    }
...
extern "C" void absorption_spectrum(
    bool particles, size_t N, double *pos, double *vel,
    double *vpec_z, // DS: LOS peculiar velocity array
    double *hsml, double *n, double *temp,
    double *rho,        // DS: density
    double *metal_frac, // SA: metal mass fraction
    double *los_pos, double *vel_extent, size_t Nbins, double b_0,
    double *v_turb, double Xsec, double Gamma, double *taus, double *los_dens,
    ...
    double *v_lims, double *column, const char *kernel_, double periodic) {
  if (particles) {
    return _absorption_spectrum<true>(
        N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos,
        vel_extent, Nbins, b_0, v_turb, Xsec, Gamma, taus, los_dens,
        los_dens_phys, los_metal_frac, los_temp, los_vpec, v_lims, column,
        kernel_, periodic);
    ...
    return _absorption_spectrum<false>(
        N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos,
        vel_extent, Nbins, b_0, v_turb, Xsec, Gamma, taus, los_dens,
        los_dens_phys, los_metal_frac, los_temp, los_vpec, v_lims, column,
        kernel_, periodic);
...
extern "C" void absorption_spectrum_multiple_los(
    bool particles, size_t N, double *pos, double *vel,
    double *vpec_z, // DS LOS peculiar velocity
    double *hsml, double *n, double *temp,
    double *rho,        // DS: density
    double *metal_frac, // SA: metal mass fraction
    double *los_pos_arr, double *vel_extent, size_t Nbins, size_t Nlos,
    double b_0, double *v_turb, double Xsec, double Gamma, double *taus,
    double *los_dens,
...
{
  if (particles) {
    return _absorption_spectrum_multiple_los<true>(
        N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos_arr,
        vel_extent, Nbins, Nlos, b_0, v_turb, Xsec, Gamma, taus, los_dens,
        los_dens_phys, los_metal_frac, los_temp, los_vpec, v_lims, column,
        kernel_, periodic);
    // DS: Added rho and los_dens_phys in the arguments of the function above
    // DS: Also vpec_z and los_vpec
    ...
    return _absorption_spectrum_multiple_los<false>(
        N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos_arr,
        vel_extent, Nbins, Nlos, b_0, v_turb, Xsec, Gamma, taus, los_dens,
        los_dens_phys, los_metal_frac, los_temp, los_vpec, v_lims, column,
        kernel_, periodic);
