...
static inline bool in_lims(double v, double *v_lims) { return (v_lims[0] <= v) and (v <= v_lims[1]); }

template <bool particles>
void _absorption_spectrum_multiple_los(size_t N,        // number of particles/cells
                                       ...
                                       size_t Nbins, size_t Nlos, double b_0, double *v_turb, double Xsec, double Gamma, double *taus,
                                       double *los_dens,
      ...
      for(size_t j = 0; j < N; j++)
        {
          ...
          double FWHM_V = 0.5346 * FWHM_L + std::sqrt(0.5346 * FWHM_L * FWHM_L + FWHM_b * FWHM_b);
                      ...
                      if(Gamma > 0.0)
                        {
                          ...
                          for(int k = 2; k < K; k += 2)
                            {
                              Dtb += Voigt(v0 + k * h, sigma, Gamma) + 2. * Voigt(v0 + (k + 1) * h, sigma, Gamma);
                            }
                          Dtb *= 2;
...
void _absorption_spectrum(size_t N, double *pos, double *vel,
                          ...
                          double *rho,         // DS: density array
                          double *metal_frac,  // SA: metal mass fraction
                          double *los_pos, double *vel_extent, size_t Nbins, double b_0, double *v_turb, double Xsec, double Gamma,
...
extern "C" void absorption_spectrum(bool particles, size_t N, double *pos, double *vel,
                                    ...
                                    double *metal_frac,  // SA: metal mass fraction
                                    double *los_pos, double *vel_extent, size_t Nbins, double b_0, double *v_turb, double Xsec,
                                    ...
                                    double *v_lims, double *column, const char *kernel_, double periodic)
{
  if(particles)
    {
      return _absorption_spectrum<true>(N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos, vel_extent, Nbins, b_0, v_turb,
                                        Xsec, Gamma, taus, los_dens, los_dens_phys, los_metal_frac, los_temp, los_vpec, v_lims, column,
                                        kernel_, periodic);
    ...
    {
      return _absorption_spectrum<false>(N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos, vel_extent, Nbins, b_0, v_turb,
                                         Xsec, Gamma, taus, los_dens, los_dens_phys, los_metal_frac, los_temp, los_vpec, v_lims,
                                         column, kernel_, periodic);
      // DS: Added rho and los_dens_phys in the arguments of the function above
      // DS: Also vpec_z and los_vpec
      // SA: Added metal_frac and los_metal_frac in the arguments above
    }
...
extern "C" void absorption_spectrum_multiple_los(bool particles, size_t N, double *pos, double *vel,
                                                 double *vpec_z,  // DS LOS peculiar velocity
                                                 double *hsml, double *n, double *temp,
                                                 double *rho,         // DS: density
                                                 double *metal_frac,  // SA: metal mass fraction
                                                 double *los_pos_arr, double *vel_extent, size_t Nbins, size_t Nlos, double b_0,
...

{
  if(particles)
    {
      return _absorption_spectrum_multiple_los<true>(N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos_arr, vel_extent,
                                                     Nbins, Nlos, b_0, v_turb, Xsec, Gamma, taus, los_dens, los_dens_phys,
                                                     los_metal_frac, los_temp, los_vpec, v_lims, column, kernel_, periodic);
      // DS: Added rho and los_dens_phys in the arguments of the function above
      // DS: Also vpec_z and los_vpec
      // SA: Added metal_frac and los_metal_frac in the arguments above
    }
    ...
    {
      return _absorption_spectrum_multiple_los<false>(N, pos, vel, vpec_z, hsml, n, temp, rho, metal_frac, los_pos_arr, vel_extent,
                                                      Nbins, Nlos, b_0, v_turb, Xsec, Gamma, taus, los_dens, los_dens_phys,
                                                      los_metal_frac, los_temp, los_vpec, v_lims, column, kernel_, periodic);
      // DS: Added rho and los_dens_phys in the arguments of the function above
      // DS: Also vpec_z and los_vpec
