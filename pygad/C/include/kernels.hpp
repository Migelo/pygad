...
class Kernel
{
 public:
  ...
  Kernel();
  Kernel(KernelType type_, unsigned proj_tbl_size = 128, unsigned los_tbl_size = 32);
  Kernel(const char *name, unsigned proj_tbl_size = 128, unsigned los_tbl_size = 32);
  ...
  double value(double q, double H) const { return q < 1.0 ? value_ql1(q, H) : 0.0; }
  ...
  int los_integrals_table_size() const { return _los_integ.empty() ? 0 : _los_integ.size() * _los_integ[0].size(); }

  double proj_value_ql1(double q, double H) const;
  double proj_value(double q, double H) const { return q < 1.0 ? proj_value_ql1(q, H) : 0.0; }
 ...
 private:
  ...
  double _los_integ_loockup(int b1, int b2, double alpha_b, int x1, int x2, double alpha_x) const;
};
...
template <int d>
Kernel<d>::Kernel() : _type(UNDEFINED_KERNEL), _norm(1.0), _w([](double q) { return q; })
{
}

template <int d>
Kernel<d>::Kernel(KernelType type_, unsigned proj_tbl_size, unsigned los_tbl_size) : _type(type_), _norm(), _w()
{
  init(type_, proj_tbl_size, los_tbl_size);
}

template <int d>
Kernel<d>::Kernel(const char *name, unsigned proj_tbl_size, unsigned los_tbl_size) : _type(), _norm(), _w()
{
  ...
  for(int i = 0; i < N; i++)
    {
      ...
      gsl_integration_qag(&F, 0.0, l_max, 1e-13, 1e-13, Nws, GSL_INTEG_GAUSS61, ws, &res, &error);
      if(error > 1e-12)
      ...
      for(int j = 0; j <= M; j++)
        {
          ...
          if(l > l_max)
            l = l_max;
          gsl_integration_qag(&F, 0.0, l, 1e-13, 1e-13, Nws, GSL_INTEG_GAUSS61, ws, &res, &error);
          if(error > 1e-12)
            fprintf(stderr, "WARNING: Error in kernel integration >1e-12!\n");
          assert(res >= 0.0 && "line-of-sight integration of kernel cannot be negative!");
          _los_integ[i][j] = res;
...
template <int d>
double Kernel<d>::_los_integ_loockup(int b1, int b2, double alpha_b, int x1, int x2, double alpha_x) const
{
