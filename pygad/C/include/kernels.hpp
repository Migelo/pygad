...
private:
  ...
  double _los_integ_loockup(int b1, int b2, double alpha_b, int x1, int x2,
...
template <int d>
Kernel<d>::Kernel(KernelType type_, unsigned proj_tbl_size,
                  unsigned los_tbl_size)
    : _type(type_), _norm(), _w() {
  init(type_, proj_tbl_size, los_tbl_size);
}

template <int d>
Kernel<d>::Kernel(const char *name, unsigned proj_tbl_size,
                  unsigned los_tbl_size)
    : _type(), _norm(), _w() {
...
template <int d>
void Kernel<d>::init(KernelType type_, unsigned proj_tbl_size,
                     unsigned los_tbl_size) {
  _type = type_;
  ...
  case QUARTIC_SPLINE:
    ...
    case 1:
      ...
      break;
    case 2:
      _norm = 46875.0 / (2398.0 * M_PI);
      break;
    case 3:
      _norm = 15625.0 / (512.0 * M_PI);
      break;
    }
    break;
  ...
  case QUINTIC_SPLINE:
    ...
    case 1:
      ...
      break;
    case 2:
      _norm = 15309.0 / (478.0 * M_PI);
      break;
    case 3:
      _norm = 2187.0 / (40.0 * M_PI);
      break;
    }
    break;
...
template <int d>
void Kernel<d>::init(const char *name, unsigned proj_tbl_size,
                     unsigned los_tbl_size) {
...
template <int d>
void Kernel<d>::require_table_size(unsigned proj_tbl_size,
                                   unsigned los_tbl_size) {
...
template <int d> void Kernel<d>::generate_projection(int N) {
  assert(0 < N);
  ...
  for (int i = 0; i < N; i++) {
    ...
    gsl_integration_qag(&F, 0.0, l_max, 1e-13, 1e-13, Nws, GSL_INTEG_GAUSS61,
                        ws, &res, &error);
    if (error > 1e-12)
      fprintf(stderr, "WARNING: Error in kernel integration >1e-12!\n");
    assert(res >= 0.0 &&
           "line-of-sight integration of kernel cannot be negative!");
    _proj[i] = 2.0 * res;
  }
  gsl_integration_workspace_free(ws);
}

template <int d> void Kernel<d>::generate_los_integrals(int N, int M) {
  assert(0 < N);
  assert(0 < M);
  ...
  for (int i = 0; i <= N; i++) {
    ...
    for (int j = 0; j <= M; j++) {
      ...
      if (l > l_max)
        l = l_max;
      gsl_integration_qag(&F, 0.0, l, 1e-13, 1e-13, Nws, GSL_INTEG_GAUSS61, ws,
                          &res, &error);
      if (error > 1e-12)
        fprintf(stderr, "WARNING: Error in kernel integration >1e-12!\n");
      assert(res >= 0.0 &&
...
template <int d> double Kernel<d>::proj_value_ql1(double q, double H) const {
  assert(0.0 <= q and q <= 1.0);
...
template <int d>
double Kernel<d>::_los_integ_loockup(int b1, int b2, double alpha_b, int x1,
                                     int x2, double alpha_x) const {
  ...
  double I_b1 =
      (1.0 - alpha_x) * _los_integ[b1][x1] + alpha_x * _los_integ[b1][x2];
  double I_b2 =
      (1.0 - alpha_x) * _los_integ[b2][x1] + alpha_x * _los_integ[b2][x2];
  return (1.0 - alpha_b) * I_b1 + alpha_b * I_b2;
}
template <int d>
double Kernel<d>::los_integ_value(double b, double x, double y,
                                  double H) const {
  assert(d == 3);
