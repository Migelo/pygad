...
public:
  ...
  std::vector<size_t> ngbs_within_if(const double r[d], double H,
                                     const double *pos, const double periodic,
                                     F cond) const;
  ...
  template <typename F>
  std::pair<size_t, double> next_ngb_with(const double r[d], const double *pos,
                                          const double periodic, F cond) const;
...
extern "C" void fill_octree(void *const octree, size_t N,
...
extern "C" size_t get_octree_node_count(const void *const octree,
                                        int count_non_leaves);
extern "C" int get_octree_in_region(const void *const octree,
                                    const double r[3]);
...
extern "C" void get_octree_ngbs_within(void *const octree, const double r[3],
                                       double H, size_t max_ngbs, size_t *ngbs,
                                       size_t *N_ngbs, const double *const pos,
                                       const double periodic,
                                       const int32_t *cond);
extern "C" void get_octree_ngbs_SPH(void *const octree, const double r[3],
                                    const double *const H, size_t max_ngbs,
                                    size_t *ngbs, size_t *N_ngbs,
                                    const double *const pos,
                                    const double periodic, const double tol);
extern "C" size_t get_octree_next_ngb(void *const octree, const double r[3],
                                      const double *const pos,
                                      const double periodic,
                                      const int32_t *cond);
...
template <int d>
Tree<d>::Tree()
    : _center(), _side_2(), _leaf(true), _tot_part(0), _num_child(0), _max_H(),
      _child() {}
...
template <int d> bool Tree<d>::is_in_region(const double pos[d]) const {
 ...
 */
template <int d> unsigned Tree<d>::get_oct(const double pos[d]) const {
  unsigned oct = 0u;
  for (int i = 0; i < d; i++) {
...
template <int d> void Tree<d>::fill_max_H(const double *H) {
  _max_H = 0.0;
  if (_leaf) {
...
template <int d> void Tree<d>::fill_max_H(double H) {
  _max_H = H;
  if (not _leaf) {
    for (unsigned i = 0; i < NC; i++) {
      Tree<d> *node = _child.node[i];
...
template <int d> size_t Tree<d>::count_nodes(bool count_non_leaves) const {
  if (_leaf)
    return 1;
  size_t nodes = 0;
  for (unsigned i = 0; i < NC; i++) {
...
template <int d> size_t Tree<d>::count_particles() const {
  if (_leaf)
    return _num_child;
  size_t parts = 0;
  for (unsigned i = 0; i < NC; i++) {
...
template <int d> int Tree<d>::get_max_depth() const {
  if (_leaf)
    return 0;
  int max_depth = 0;
  for (unsigned i = 0; i < NC; i++) {
...
template <typename F>
std::vector<size_t>
Tree<d>::ngbs_within_if(const double r[d], double H, const double *pos,
                        const double periodic, F cond) const {
  ...
  if (_leaf) {
    ...
    for (const auto node : _child.node) {
      if (node) {
        ...
        if (TREE_NODE_OPEN_TOL * max_d <
            side_2_H) { // open node and add neighbors therein
          std::vector<size_t> node_ngbs =
              node->ngbs_within_if(r, H, pos, periodic, cond);
    ...
    for (const auto node : _child.node) {
      if (node) {
        ...
        if (TREE_NODE_OPEN_TOL * max_d < node->_side_2 + node->_max_H + tol) {
          std::vector<size_t> node_ngbs =
              node->ngbs_SPH(r, H, pos, periodic, tol);
          ngb_idx.insert(ngb_idx.end(), node_ngbs.begin(), node_ngbs.end());
        }
      }
    }
...
Tree<d>::next_ngb_with(const double r[d], const double *pos,
                       const double periodic, F cond) const {
