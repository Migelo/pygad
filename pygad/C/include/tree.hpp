...
class Tree
{
 public:
  ...
  template <typename F>
  std::vector<size_t> ngbs_within_if(const double r[d], double H, const double *pos, const double periodic, F cond) const;
  std::vector<size_t> ngbs_within(const double r[d], double H, const double *pos, const double periodic) const
  {
    return ngbs_within_if(r, H, pos, periodic, [](size_t i) { return true; });
  }
  std::vector<size_t> ngbs_SPH(const double r[d], const double *H, const double *pos, const double periodic, const double tol) const;
  template <typename F>
  std::pair<size_t, double> next_ngb_with(const double r[d], const double *pos, const double periodic, F cond) const;
...
template <int d>
Tree<d>::Tree() : _center(), _side_2(), _leaf(true), _tot_part(0), _num_child(0), _max_H(), _child()
{
}

template <int d>
Tree<d>::Tree(const double center_[d], double side_2_)
    : _center(), _side_2(side_2_), _leaf(true), _tot_part(0), _num_child(0), _max_H(), _child()
{
  for(int i = 0; i < d; i++)
    _center[i] = center_[i];
}
...
template <int d>
template <typename F>
std::vector<size_t> Tree<d>::ngbs_within_if(const double r[d], double H, const double *pos, const double periodic, F cond) const
{
          ...
          if(node)
            {
              ...
              if(TREE_NODE_OPEN_TOL * max_d < side_2_H)
                {  // open node and add neighbors therein
...
template <int d>
std::vector<size_t> Tree<d>::ngbs_SPH(const double r[d], const double *H, const double *pos, const double periodic,
...
template <int d>
template <typename F>
std::pair<size_t, double> Tree<d>::next_ngb_with(const double r[d], const double *pos, const double periodic, F cond) const
{
