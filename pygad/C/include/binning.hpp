...
extern "C" void sph_bin_3D(size_t N, double *pos, double *hsml, double *dV,
                           double *qty, double *extent, size_t Npx[3],
                           double *grid, const char *kernel_, double periodic);

extern "C" void sph_bin_3D_nonorm(size_t N, double *pos, double *hsml,
                                  double *dV, double *qty, double *extent,
                                  size_t Npx[3], double *grid,
                                  const char *kernel_, double periodic);

extern "C" void sph_3D_bin_2D(size_t N, double *pos, double *hsml, double *dV,
                              double *qty, double *extent, size_t Npx[2],
                              double *grid, const char *kernel_,
                              double periodic);

extern "C" void sph_3D_bin_2D_nonorm(size_t N, double *pos, double *hsml,
                                     double *dV, double *qty, double *extent,
                                     size_t Npx[2], double *grid,
                                     const char *kernel_, double periodic);

extern "C" void bin_sph_along_line(size_t N, double *pos, double *hsml,
                                   double *dV, double *qty, double *los,
                                   double *extent, size_t Npx, double *line,
                                   const char *kernel_, double periodic);

// Mind the reversed indexing of i_min, i_max, and i due to performace at
// accessing array elements in reversed loop order in
// nested_loops<2>::do_loops(...)! Index n in i,i_min,i_max is index k=(d-1)-n
// for grid_r, Npx, res, etc... (and vice versa).

// helper funktion templates
template <int d> bool out_of_grid(const size_t *i, const size_t *Npx) {
  for (int k = 0; k < d; k++) {
...
template <int d>
bool extents_out_of_grid(const size_t *i_min, const size_t *i_max,
                         const size_t *Npx) {
  for (int n = 0; n < d; n++) {
...
void bin_sph(size_t N, double *pos, double *hsml, double *dV, double *qty,
             double *extent, size_t Npx[d], double *grid, const char *kernel_,
             double periodic) {
...
#pragma omp parallel for default(shared) schedule(dynamic, 10)
  for (size_t j = 0; j < N; j++) {
    ...
    for (int k = 0; k < d; k++) {
      i_min[(d - 1) - k] =
          std::max<double>((rj[k] - extent[2 * k] - hj) / res[k], 0.0);
      i_min[(d - 1) - k] = std::min(i_min[(d - 1) - k], Npx[k]);
      i_max[(d - 1) - k] =
          std::min<size_t>((rj[k] - extent[2 * k] + hj) / res[k] + 1.0, Npx[k]);
    }
    ...
    if (norm) {
      ...
      if (hj > H_lim_out_of_grid * res_min and
          extents_out_of_grid<d>(i_min, i_max, Npx)) {
        S = 1.0;
      } else {
        S = 0.0;
        size_t i[d];
...
template <int d>
void bin_sph_line(size_t N, double *pos, double *hsml, double *dV, double *qty,
                  double *los, double *extent, size_t Npx, double *line,
                  const char *kernel_, double periodic) {
...
#pragma omp parallel for default(shared) schedule(dynamic, 10)
  for (size_t j = 0; j < N; j++) {
    ...
    for (size_t i = i_min; i < i_max; i++) {
      ...
      double Wj =
          kernel.los_integ_value(b / hj, (z1 - z) / hj, (z2 - z) / hj, hj);
