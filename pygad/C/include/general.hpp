...
template <unsigned power> constexpr int pow_int(int base) {
  return pow_int<power - 1>(base) * base;
}
template <> constexpr int pow_int<0>(int base) { return 1; }
...
template <int d> double dist2(const double x1[d], const double x2[d]) {
  double d2 = 0.0;
  for (int i = 0; i < d; i++) {
    double di = x1[i] - x2[i];
    d2 += di * di;
...
template <int d> double dist(const double x1[d], const double x2[d]) {
...
template <int d> double dist_max(const double x1[d], const double x2[d]) {
  double dist = 0.0;
  for (int i = 0; i < d; i++) {
...
template <int d> double norm(const double x[d]) {
  double norm = 0.0;
  for (int i = 0; i < d; i++)
    norm += x[i] * x[i];
  return sqrt(norm);
