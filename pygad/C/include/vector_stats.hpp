...
template <typename T> T mean(const std::vector<T> &X, const std::vector<T> &w) {
...
template <typename T> T variance(const std::vector<T> &X) {
  T var(0);
  T EX = mean(X);
  for (const T &x : X) {
    T tmp = x - EX;
...
template <typename T> T stddev(const std::vector<T> &X) {
