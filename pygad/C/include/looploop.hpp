...
// innermost levels have a function passed, if more are provided the last ones
// are just without effect. If, however, exactly to functions are passed, the
// first one is executed at each level except the innermost level, where the
// second function is executed. Call signature of the functions:
//      (unsigned n, idx_t *i, idx_t *i_min, idx_t *i_max)
template <unsigned n> struct nested_loops {
  template <typename F, typename... functions>
  static void do_loops(idx_t *i, idx_t *i_min, idx_t *i_max, F f,
                       functions... funcs) {
    for (i[n - 1] = i_min[n - 1]; i[n - 1] < i_max[n - 1]; i[n - 1]++) {
      if (sizeof...(functions) == n - 1 or
          (sizeof...(functions) == 1 and n == 2)) {
        f(n - 1, i, i_min, i_max);
...
template <> struct nested_loops<1> {
  template <typename F, typename... functions>
  static void do_loops(idx_t *i, idx_t *i_min, idx_t *i_max, F f,
                       functions... funcs) {
...
template <> struct nested_loops<0> {
  template <typename F, typename... functions>
  static void do_loops(idx_t *i, idx_t *i_min, idx_t *i_max, F f,
                       functions... funcs) {
    f(0, i, i_min, i_max);
