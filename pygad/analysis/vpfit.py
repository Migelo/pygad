...
__all__ = ['fit_profiles','model_tau','find_regions','EquivalentWidth','periodic_wrap','periodic_unwrap_wavelength','write_spectrum','write_lines','plot_fit']
...
def fit_profiles(line, l, flux, noise, chisq_lim=2.0, max_lines=10, mode='Voigt', logN_bounds=[12,19], b_bounds=[5,200]):
    '''
    ...
    if isinstance(line, str):
        line = lines[line]
    l0 = line['l']
    if isinstance(l,np.ndarray) or l.units in [1, None]:  # set units of l to Angstrom if none supplied
    ...
    def _chisq(p, l, flux, noise, mode):  # reduced chisq
        return np.sum((_tau_to_flux(model_tau(line, p, l, mode)) - flux) ** 2 / noise ** 2) / len(l)

    def _add_line(p, bnd, l, flux, l0, mode):  # adds N, b, l for a new line
        l_bounds = [l[1],l[-2]]
        b_guess = (l_bounds[1]-l_bounds[0]) / float(l0.split()[0]) * 3.e5 / 5.  # first guess at b
        b_guess = max(2*b_bounds[0], 0.5*min(b_bounds[1], b_guess))
        if len(p) == 0:
            resid = flux 
            n_guess = 14.-resid[np.argmin(resid)]  # first guess at logN
            p = np.array([n_guess])  # rough guess of logN
        else:
            resid = 1. + flux - _tau_to_flux(model_tau(line, p, l, mode)) # residual spectrum
            n_guess = 14.-resid[np.argmin(resid)]  # first guess at logN
    ...
    line_list = {'region': np.array([]), 'l': np.array([]), 'dl': np.array([]), 'b': np.array([]), 'db': np.array([]), 'N': np.array([]), 'dN': np.array([]), 'EW': np.array([])}
    ...
    for ireg in range(len(regions_l)):
        ...
        while n_lines < max_lines:
            ...
            soln = minimize(chisq_fcn, params, bounds=bounds, args=(l_reg, f_reg, n_reg, mode), options={'maxiter':100})
            ...
            if params[n_lines*3-1] < l[2] or params[n_lines*3-1] > l[-2]:
                ...
                chisq_soln = _chisq(params, l_reg, f_reg, n_reg, mode)
            '''
            if chisq_soln < chisq_accept: break
            if chisq_lim < 0: chisq_accept += 0.1

        if chisq_soln > chisq_accept:  # try to go back to previous best solution 
            if environment.verbose == environment.VERBOSE_TACITURN:
                print('Region %d uncoverged with %d lines, chisq=%g; retrying with %d'%(ireg,n_lines,chisq_soln,best_nlines))
            n_lines = best_nlines
            ...
            if chisq_soln > chisq_accept: 
                if environment.verbose >= environment.VERBOSE_TACITURN:
                    print('WARNING: region %d has large chisq=%g; check fit'%(ireg,chisq_soln))


        params += 0.02*(2*np.random.rand(len(params))-1)  # jiggle params and refit to compute hessian
        soln = minimize(chisq_fcn, params, args=(l_reg, f_reg, n_reg, mode), method='BFGS', options={'maxiter':100})
        cov = soln.hess_inv  # covariance matrix of final soluiton

        # append lines in this region onto line list
        if environment.verbose >= environment.VERBOSE_TACITURN:
            print('region %d (%g-%g): chisq= %g with %d lines'%(ireg,regions_l[ireg,0],regions_l[ireg,1],chisq_soln,int(len(params)/3)))
        for ip in range(n_lines):
            ...
            tau_line = model_tau(line, [params[ip*3],params[ip*3+1],params[ip*3+2]], l_reg, mode)
...
def model_tau(line, p, l, mode='Voigt'):
    ...
    if len(p) == 0: return total_tau  # no lines yet, return zeros
    for ip in range(int(len(p)/3)):
        _, tau = line_profile(line, 10**p[ip*3], b=p[ip*3+1], l0=p[ip*3+2], l=l, mode=mode)
...
def find_regions(wavelengths, fluxes, noise, min_region_width=2, N_sigma=5.0, extend=False):
    """
...
def write_spectrum(spec_name, line, LOS_pos, lambda_rest, redshift, vels, fluxes, taus, noise, col_dens, phys_dens, temps, mets, vpec, overwrite=True):
    """
    ...
    if os.path.isfile(spec_name) and not overwrite:
        if environment.verbose >= environment.VERBOSE_TACITURN:
            print('WARNING: write_spectrum() failed: File %s exists, and overwrite set to False'%spec_name)
        return 

    waves = lambda_rest * (redshift + 1.) * (1. + vels / c)
    mets = np.log10(np.where(mets<1.e-10, 1.e-10, mets))  # turn into log10(Z)
    if len(LOS_pos) == 2:
        LOS_pos = np.append(np.array(LOS_pos),-1.)  # assumes if only 2 values are provided, they are (x,y), so we add -1 for z.
...
def plot_fit(ax, waves, flux, noise, line_list, line, starting_pixel=0, show_plot=True, show_label=True):
    """
